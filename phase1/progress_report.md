...
4. Move Framework.DataAccess.WriteFile class to the Interface Adapter layer and make them be the gateway classes.
...
#### Liskov substitution: 
Entity.User.User is the parent class of Entity.User.NormalUser and Admin Entity.User.User. All features in Entity.User.User are passed to its child class, and we can replace Entity.User.User with either Entity.User.NormalUser or Entity.User.AdminUser and the program should not disrupt the behavior of our program. Also, we have a Framework.DataAccess.WriteFile class which is the parent class of Framework.DataAccess.WriteMovie, Framework.DataAccess.WriteUser and Framework.DataAccess.WriteReview. These three classes also inherited all features from Framework.DataAccess.WriteFile. Although they are in charge of different aspects of the project, they perform their functions in similar ways. Except for that, a class called NormalController acts as the parent class of other controllers for movie, coin, user and review. These controllers each have their own type of object that they are in charge of but they all have similar features that their parent class, NormalController, passed to them.
...
### Packaging
The project is packaged by layers. Four layers: Entity, Use Cases, Interface Adapter and Framework are used to name the four packages that contain the body of our codes. For each layer, lower level packages are contained and classes at the same layer are separated by their functionaries. Entity contains three entities: Entity.User.User (which is also a package containing Entity.User.NormalUser and Entity.User.AdminUser), Entity.Movie and Entity.Review. UseCases contain Manager classes. Manager package contains four managers for movie, user, review and coin. The next layer package is InterfaceAdapter, which contains controllers package for movie, review, user and coin, inputprocessor and instancemain; and also Framework.DataAccess.WriteFile package contains Framework.DataAccess.WriteMovie, Framework.DataAccess.WriteUser, Framework.DataAccess.WriteReview and a Framework.DataAccess.WriteFile class. The outermost layer is the Framework, which contains UI and GUI, two packages. However, some errors still occur due to the fact that we are unable to import and use every class across packages correctly.
...
After putting classes in packages, errors occurred as some classes cannot be called due to the fact that they are not in the same package, but importing them would change their type. For example, Entity.Movie type now changed to Entity.Entity.Movie type. Is there any way to avoid these errors?
To save data from Hashmap and keep its structure, we used properties for Framework.DataAccess.WriteMovie, however, when running, a line of “ava.io.FileOutputStream@dc24521Check your review” appeared. Though it does not affect the code running, it is annoying. Is there any way to delete that?
...
Yuxuan Li worked on updating a controller class AdminInputProcessor and implementing a new use case class UseCase.CoinManager.

Jacquelyn Wang was responsible for splitting the long class NormalInputProcessor in phase 0, and updating the UseCase.ReviewManager NormalController, NormalCCoin, NormalCUser, NormalCMovie for the altered file structure and the newly added coin feature.

Jiaxi Li worked on GUI design, fix and update some features in Entity.Review and Write Entity.Review class, and complete tests for Entity.Review Class.

Jing Pan (Ella) worked on three Framework.DataAccess.WriteFile Classes, changing the layer of Framework.DataAccess.WriteFile Classes from use case to gateway and changing the way for program to create the entity instance; creating a class called InstanceMain to store all the instance will be needed when program runs; Changing the corresponding places in all classes that how all classes call other classes when they need; Updating the feature of coin system in Entity.User.User Classes and UseCase.UserManager.

Andy worked on designing packages and fixed some features on Entity.Movie&Framework.DataAccess.WriteMovie, designed tests for movie parts, and worked on design documents.
...
Yuxuan Li will be working on implementing new functions supporting Entity.User.AdminUser to delete movies, modify information of movies and delete reviews.
