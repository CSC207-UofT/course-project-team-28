...
### CRC Model Summary
According to our CRC model, we have 16 classes in total, each class is assigned with a function according to the clean architecture. Entity includes Entity.User.User, Normal Entity.User.User, Admin Entity.User.User, Entity.Movie, Entity.Review, which is the innermost part of the code, that creates new objects when other outer classes calls. Use cases are Entity.Movie Manager, Entity.Review Manager, Entity.User.User Manager, Writeflie, Write Entity.Review, Write Entity.Movie, Write Entity.User.User, and Framework.DataAccess.IgnoreFirstLineBufferedWriter. The managers pack the inputs and interact with write classes to create/write/read the file. There are two controllers, AdminInputProcessor and NormalInuptProcessor. Each is responsible for receiving and processing input from the UI. The UserInterface is the command line interface which receives input.
...
and we do have several unit tests for Entity.User.NormalUser and both passed. We also extensively tested our program in the shell.
...
In this project, Jing Pan completed the Entity.User.User, Entity.User.AdminUser, Entity.User.NormalUser UseCase.UserManager, Framework.DataAccess.WriteFile and Framework.DataAccess.WriteUser. Jacquelyn Wang worked on the UseCase.ReviewManager, and also completed NormalCUser with Yuxuan Li, and Yuxuan also completed AdminInputProcessor and the unittest of Entity.User.NormalUser Class. Fan Pan and Haitao Qiu worked together for UserInterface. Haitao Qiu also completed UseCase.MovieManager class. Jiaxi Li finished Entity.Review and Framework.DataAccess.WriteReview classes. Hao Han worked for classes Entity.Movie, Framework.DataAccess.WriteMovie and the helper class called Framework.DataAccess.IgnoreFirstLineBufferedWriter.
...
We also want to equip Entity.User.AdminUser with the methods of deleting movies and deleting reviews. In the future, normal users can also upload movies if these movies have not been uploaded yet.
